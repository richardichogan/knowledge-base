  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.success && res.data.length > 0 ? res.data : STATIC_PROJECTS.map((project) => ({
        ...
        hasIcaDocumentCollection: project.hasIcaDocumentCollection ?? false,
        icaDocumentCollectionName: project.icaDocumentCollectionName ?? '',
        icaDocumentCollectionId: project.icaDocumentCollectionId ?? '',
        links: project.links ?? [],
        tags: project.tags ?? [],
        createdAt: '',
        updatedAt: '',
